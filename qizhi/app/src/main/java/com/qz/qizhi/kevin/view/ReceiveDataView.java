package com.qz.qizhi.kevin.view;
...
public class ReceiveDataView extends LinearLayout {
	...
	public ReceiveDataView(Context context, AttributeSet attrs) {
		...
		LayoutInflater.from(context).inflate(Res.layout.receive_data_view,
				this, true);
		lengthTxt = (TextView) this.findViewById(Res.id.lengthTxt);
		countTimesTxt = (TextView) this.findViewById(Res.id.countTimesTxt);
		messageTxt = (TextView) this.findViewById(Res.id.messageTxt);
		messageBG = (ScrollView) this.findViewById(Res.id.messageBg);
	...
	public void changeEditBackground(boolean boo) {
		if (boo) {
			this.messageBG
					.setBackgroundResource(Res.drawable.corner_pressed_round_body);
		} else {
			this.messageBG
					.setBackgroundResource(Res.drawable.corner_round_body);
